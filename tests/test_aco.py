...
def test_select_path_returns_valid_route():
    ...
    path = select_path(graph, "M", "S6", 2.0, 1.0, rng=random.Random(123))
...
def test_select_path_returns_none_for_missing_node():
    ...
    assert select_path(graph, "M", "UNKNOWN", 2.0, 1.0) is None
...
def test_select_path_rejects_invalid_exploration_rate():
    ...
    with pytest.raises(ValueError):
        select_path(graph, "M", "S6", 2.0, 1.0, exploration_rate=1.5)
