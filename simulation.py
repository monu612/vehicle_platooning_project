...
def run_simulation(
    ...
    aco_paths_cache: dict[str, list[list[str]]] = {}
    for destination in DESTINATIONS:
        ...
        try:
            aco_paths_cache[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=MAX_PATH_LENGTH))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            aco_paths_cache[destination] = []
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=aco_paths_cache.get(destination),
            )
