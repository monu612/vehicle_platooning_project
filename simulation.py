...
def run_simulation(
    ...
    # Pre-compute all simple paths up to cutoff=4 on the pristine static topology
    # to avoid expensive nx.all_simple_paths calls in the dynamic graph per iteration.
    aco_precomputed_paths: dict[str, list[list[str]]] = {}
    for destination in DESTINATIONS:
        try:
            aco_precomputed_paths[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=4))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
