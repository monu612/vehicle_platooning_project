...
def run_simulation(
    ...
    # Pre-compute all simple paths for ACO to avoid recalculating on mutated graphs.
    all_routes: dict[str, list[list[str]]] = {}
    ...
    for destination in DESTINATIONS:
        ...
        try:
            all_routes[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=MAX_PATH_LENGTH))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            all_routes[destination] = []
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=all_routes.get(destination),
            )
