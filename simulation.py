...
def run_simulation(
    ...
    # Pre-compute all simple paths for ACO to avoid redundant recalculation
    # on the pristine master graph G.
    all_simple_paths: dict[str, list[list[str]]] = {}
    ...
    for destination in DESTINATIONS:
        ...
        try:
            all_simple_paths[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=MAX_PATH_LENGTH))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=all_simple_paths.get(destination),
            )
