...
def run_simulation(
    ...
    master_paths: dict[str, list[list[str]]] = {}
    for destination in DESTINATIONS:
        ...
        try:
            master_paths[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=4))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            master_paths[destination] = []
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=master_paths.get(destination),
            )
