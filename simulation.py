...
def run_simulation(
    ...
    # Pre-compute all simple paths on the pristine master graph for ACO
    # This avoids expensive path recalculations on every iteration, and paths
    # that are broken due to temporary link failures can simply be filtered later.
    all_paths: dict[str, list[list[str]]] = {}
    for destination in DESTINATIONS:
        try:
            all_paths[destination] = list(nx.all_simple_paths(G, "M", destination, cutoff=MAX_PATH_LENGTH))
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            all_paths[destination] = []
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=all_paths[destination],
            )
