...
def run_simulation(
    ...
    # Performance optimization: Pre-compute all simple paths for ACO once on the static base topology.
    # During dynamic simulation iterations, we will only filter these paths to verify edge existence
    # rather than invoking expensive `nx.all_simple_paths` on the perturbed graph.
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=aco_precomputed_paths[destination],
            )
