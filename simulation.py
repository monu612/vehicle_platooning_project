...
def run_simulation(
    ...
    # Pre-compute simple paths for ACO once
    all_simple_paths_precomputed: dict[str, list[list[str]]] = {}
    for destination in DESTINATIONS:
        ...
        try:
            all_simple_paths_precomputed[destination] = list(
                nx.all_simple_paths(G, "M", destination, cutoff=MAX_PATH_LENGTH)
            )
        except (nx.NetworkXNoPath, nx.NodeNotFound):
            all_simple_paths_precomputed[destination] = []
    ...
    for i in range(runs):
        ...
        for destination in DESTINATIONS:
            ...
            path = select_path(
                ...
                rng=rng,
                precomputed_paths=all_simple_paths_precomputed[destination]
            )
