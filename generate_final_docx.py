...
replacements = {
    "TITLE OF THE PROJECT": "Adaptive Smart ACO Based Robust Communication "
    ...
    "Write your abstract here.": (
        "This project addresses the critical challenge of reliable vehicle-to-vehicle "
        "(V2V) communication in next-generation autonomous vehicle platooning. "
        "Existing star topology communication architectures suffer from single "
        "points of failure, high latency, and significant performance degradation "
        "under network congestion or link failures. To overcome these limitations, "
        "we propose a novel decentralised communication architecture combining a "
        "Spider-Web Mesh Topology with Adaptive Smart Ant Colony Optimization "
        "(AS-ACO) for dynamic routing. The spider-web topology ensures high path "
        "redundancy and fault tolerance. Unlike classical ACO which uses fixed "
        "parameters, our Adaptive Smart ACO dynamically adjusts routing parameters "
        "(α, β, ρ) in real time based on network congestion, link reliability, and "
        "network instability. This adaptive intelligence continuously prevents "
        "network loops and ensures optimal convergence under high stress. Extensive "
        "simulations demonstrate that the proposed AS-ACO-driven spider-web "
        "architecture achieves near 100% packet delivery ratio, significantly "
        "reduces communication latency during unstable network conditions, and "
        "maintains robust connectivity even when up to 30% of communication links "
        "fail simultaneously. This report comprehensively details the mathematical "
        "models, algorithmic design, software architecture, and performance "
        "evaluations of the implemented system."
    ),
    "Keywords: Write your keywords here separated by comma.": (
        "Keywords: Vehicle Platooning, Adaptive Smart ACO, AS-ACO, Dynamic "
        "Parameter Optimization, Spider-Web Topology, V2V Communication, Fault "
        "Tolerance, Dynamic Routing, Autonomous Vehicles."
    )
}
