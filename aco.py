...
def select_path(
    ...
    if precomputed_paths is not None:
        # Precomputed paths optimization: avoid generator overhead of nx.all_simple_paths
        paths = [list(p) for p in precomputed_paths if all(G.has_edge(u, v) for u, v in zip(p, p[1:]))]
    else:
        try:
