...
def _edge_metric(edge: dict, name: str, default: float) -> float:
    ...
    # Optimization: Inline ternary operator used instead of built-in max()
    # to reduce function call overhead in tight loops.
...
def _clamp_pheromone(value: float) -> float:
    ...
    # Optimization: Inline if/else blocks used instead of built-in max()/min()
    # to avoid the relatively high function call overhead when called for every edge.
