...
def _edge_metric(edge: dict, name: str, default: float) -> float:
    # ⚡ Bolt: Replace max() with ternary operator to reduce function call overhead in tight loops
...
def _clamp_pheromone(value: float) -> float:
    ...
    # ⚡ Bolt: Replace max/min with if/elif/else block to reduce function call overhead
    if value > PHEROMONE_MAX:
        return PHEROMONE_MAX
    elif value < PHEROMONE_MIN:
