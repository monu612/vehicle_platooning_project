...
def _edge_metric(edge: dict, name: str, default: float) -> float:
    ...
    # Perf: Use inline ternary instead of max() to avoid function call overhead in tight loop
...
def _clamp_pheromone(value: float) -> float:
    ...
    # Perf: Use if/else instead of max(min()) to avoid function call overhead in tight loop
