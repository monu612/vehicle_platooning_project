    ...
    if horizontal:
        ...
        for i, step in enumerate(steps):
            ...
            ax.text(
                ...
                fontweight='bold')
            if i < len(steps) - 1:
                ax.arrow(2.5, y * 2, 0, -0.6, head_width=0.2,
                         head_length=0.2, fc=CYAN, ec=CYAN)
        ax.set_xlim(-1, 6)
        ax.set_ylim(-1, len(steps) * 2)
    return save_fig(fig, filename)


arch_img = draw_flowchart(["Network State Monitoring",
                           "Adaptive Parameter Engine",
                           "Smart Route Selection",
                           "Dynamic Pheromone Update",
                           "Packet Delivery"],
                          "architecture.png")
algo_img = draw_flowchart(["Initialize Graph",
                           "Monitor Network",
...
math_alpha = render_math(
    r"\alpha_t = \alpha_0 + k_1 \times FailureRate",
    "math_alpha.png")
math_beta = render_math(
    r"\beta_t = \beta_0 + k_2 \times AvgCongestion",
    "math_beta.png")
math_rho = render_math(
    r"\rho_t = \rho_0 + k_3 \times NetworkInstability",
    "math_rho.png")
math_instab = render_math(
    r"Instability = 1 - \frac{AvgReliability}{AvgCongestion}",
    "math_instab.png")
math_score = render_math(
    r"Score(P) = \prod_{(u,v)\in P}\tau(u,v)^\alpha \times \left(\frac{r(u,v)}{w(u,v)\times g(u,v)}\right)^\beta",
    "math_score.png",
    fontsize=20)
math_update = render_math(
    r"\tau = (1-\rho)\tau + \frac{R}{L_P}",
    "math_update.png")
math_pdr = render_math(
    r"PDR = \frac{PacketsReceived}{PacketsSent}",
    "math_pdr.png")
...
chart_pdr = render_bar_chart(
    "Packet Delivery Ratio (PDR)", [
        0.65, 0.88, 0.98], "chart_pdr.png")
chart_lat = render_bar_chart(
    "Latency (ms)", [
        45.2, 32.1, 21.5], "chart_lat.png")
chart_red = render_bar_chart(
    "Redundancy (Hops)", [
        3.8, 2.5, 2.1], "chart_red.png")
